...
def R_MLR(para):
    ...
    class MLPResNet(nn.Module):
        def __init__(self, in_dim, num_classes, width=256, depth=4):
            super().__init__()
            self.inp = nn.Sequential(nn.Linear(in_dim, width), nn.ReLU(), nn.LayerNorm(width))
            self.blocks = nn.Sequential(*[MLPResBlock(width, hidden=width*2) for _ in range(depth)])
    ...
    model = MLPResNet(d, num_class, width=256, depth=4)
    ...
    if para.optimizer == 'adam' or para.optimizer == 'adagrad':
        ...
        for epoch in range(para.num_epoch):
            ...
            for X_batch, y_batch in train_loader:
                ...
                y_true = torch.argmax(y_batch_one_hot, dim=1)
